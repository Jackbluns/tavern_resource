...
.roleplay_checkbox_slider::-webkit-scrollbar {
    height: 6px;
}

.roleplay_checkbox_slider::-webkit-scrollbar-track {
    background: rgba(0, 0, 0, 0.1);
    border-radius: 3px;
}

.roleplay_checkbox_slider::-webkit-scrollbar-thumb {
    background: rgba(160, 160, 160, 0.4);
    border-radius: 3px;
    transition: background 0.3s ease;
}

.roleplay_checkbox_slider::-webkit-scrollbar-thumb:hover {
...
.roleplay_checkbox_back::before {
    ...
    width: 20px;
    height: 20px;
    background: linear-gradient(45deg, transparent 40%, rgba(200, 200, 200, 0.6) 50%, transparent 60%);
    border-radius: 50%;
    opacity: 0;
...
.roleplay_checkbox_back:hover::before {
    opacity: 0;
}
...
.roleplay_checkbox_item:focus {
    outline: 2px solid rgba(100, 150, 200, 0.8);
    outline-offset: 2px;
}

.roleplay_checkbox_back::after {
    content: "";
    position: absolute;
    top: 0;
    right: 0;
    width: 30px;
    height: 100%;
    background: linear-gradient(to left, rgba(45, 45, 45, 0.8), transparent);
    pointer-events: none;
    opacity: 0;
...
.roleplay_checkbox_slider:not(:hover)::after {
    opacity: 1;
}
</style>`:t[0].content}(),!_.isEqual(n,t)},t.extract_checkbox_element=function(e){const t=$('<div class="roleplay_checkbox">');t.append(n);const o=$('<div class="roleplay_checkbox_slider">'),r=[...e.matchAll(/(.+?)[:：]\s*(.+)/gm)].map((e=>({title:e[1],content:e[2].replace(/^\$\{(.+)\}$/,"$1").replace(/^「(.+)」$/,"$1")})));r.forEach((({title:e,content:t})=>{const r=$('<div class="roleplay_checkbox_item" tabindex="1">').on("click",(function(){!async function(e){if(e.parents(".last_mes").length>0){const t=e.find(".roleplay_checkbox_content").text().trim();if("直接发送"===a.option.input_mode)triggerSlash(`/send ${t} || /trigger`);else if("覆盖输入"===a.option.input_mode)triggerSlash(`/setinput ${t}`);else if("尾附输入"===a.option.input_mode){const e=$("#send_textarea").val();$("#send_textarea").val([e,t].join("\n")||"")[0].dispatchEvent(new Event("input",{bubbles:!0}))}}}($(this))})).append(`<span class="roleplay_checkbox_title"><strong>${e}</strong></span>`).append('<hr class="roleplay_checkbox_hr">').append(`<span class="roleplay_checkbox_content">${t}</span>`);o.append(r)});const s=$('<div class="roleplay_checkbox_back">').append(o);return t.append(s),t}}(n||(n={})),$((async()=>{await errorCatched(a.update)(),await errorCatched(n.update)(),await r(),eventOn(tavern_events.CHAT_CHANGED,errorCatched(r)),eventOn(tavern_events.CHARACTER_MESSAGE_RENDERED,errorCatched(o)),eventOn(tavern_events.MESSAGE_UPDATED,errorCatched(o)),eventOn(tavern_events.MESSAGE_SWIPED,errorCatched(o)),eventOn(tavern_events.MESSAGE_DELETED,(()=>setTimeout(errorCatched(r),1e3))),eventOn(tavern_events.WORLDINFO_UPDATED,errorCatched((async t=>{t===e&&(await a.update()||await n.update())&&await r()})))}));let l=null;function c(){null!==l&&(setTimeout(s,_.get(getVariables({type:"global"}),[e,"自动推进发送间隔"],3e3)),++l,l===_.get(getVariables({type:"global"}),[e,"自动推进循环次数"],-1)&&i())}function i(){eventRemoveListener(tavern_events.CHARACTER_MESSAGE_RENDERED,c),l=null,toastr.success("已停止自动推进",e)}$((async()=>{eventOnButton("设置循环次数",(async()=>{const t=Number(await SillyTavern.callGenericPopup("设置循环次数 (-1 为直到按下 '停止自动推进')",SillyTavern.POPUP_TYPE.INPUT,_.get(getVariables({type:"global"}),[e,"自动推进循环次数"],"-1")));-1!==t&&t<=0?toastr.error("循环次数要么是 -1, 要么是大于 0 的整数"):(insertOrAssignVariables({[e]:{自动推进循环次数:t}},{type:"global"}),-1===t?toastr.success('已设置推进次数为 -1, 即直到按下 "停止自动推进" 才会停止',e):toastr.success(`已设置推进次数为 ${t} 次`,e))})),eventOnButton("设置发送间隔",(async()=>{const t=Number(await SillyTavern.callGenericPopup("设置发送间隔 (单位: 毫秒)",SillyTavern.POPUP_TYPE.INPUT,_.get(getVariables({type:"global"}),[e,"自动推进发送间隔"],"3000")));t<=0?toastr.error("发送间隔必须大于 0"):(insertOrAssignVariables({[e]:{自动推进发送间隔:t}},{type:"global"}),toastr.success(`已设置发送间隔为 ${t} 毫秒`,e))})),eventOnButton("启动自动推进",(()=>{null===l?(l=0,c(),eventOn(tavern_events.CHARACTER_MESSAGE_RENDERED,c),toastr.success("已开启自动推进",e)):toastr.error("自动推进在之前已开启, 请先停止自动推进")})),eventOnButton("停止自动推进",i)}));
